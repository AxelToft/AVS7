...
def get_intermediate_count_accuracy(file_results):
    anno_file = open('Annotations_full.json', 'r')
    json_file = open(file_results, 'r')
    # Get video names for indexing annotations file
    anno_data = json.load(anno_file)
    ...
    for result_video in result_list['results']:
        ...
        if mode == "sum":
            if gt_video['fish_count_frames'] != result_video['fish_count_frames']:
                ...
                print("fish_count_frames not equal for video: ", result_video['video'], " ground truth sequence: ", gt_video['fish_count_frames'], " found sequence is: ",
                      result_video['fish_count_frames'])
            count_seq_count += 1
    if mode == "sum":
        return (1-count_seq_sum)/count_seq_count,wrong_count
